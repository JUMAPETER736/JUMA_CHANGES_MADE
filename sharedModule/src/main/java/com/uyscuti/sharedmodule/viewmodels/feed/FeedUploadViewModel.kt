...
import com.uyscuti.sharedmodule.model.feed.multiple_files.MixedFeedFilesClass
...
@HiltViewModel
class FeedUploadViewModel @Inject constructor(private val retrofitInstance: RetrofitInstance) :
    ViewModel() {
...
//        if (!this.mixedFeedUploadDataClass.contains(feedUploadDataClass)) {
//
//        }else {
//            Log.e(TAG, "addMixedFeedUploadDataClass: already exists", )
//        }
        this.mixedFeedUploadDataClass.add(feedUploadDataClass)
        Log.d(TAG, "addMixedFeedUploadDataClass: size: ${this.mixedFeedUploadDataClass.size}")
//        this.mixedFeedUploadDataClass.add(feedUploadDataClass)
    }

    fun getMixedFeedUploadDataClass() : MutableList<MixedFeedUploadDataClass> {
        Log.d(TAG, "addMixedFeedUploadDataClass: size: on get ${this.mixedFeedUploadDataClass.size}")
    ...
    fun updateMixedFeedUploadDataClass(position: Int, mixedFeedUploadDataClass : MixedFeedUploadDataClass) {
        this.mixedFeedUploadDataClass[position] = mixedFeedUploadDataClass
    }
    ...
    fun uploadTextFeed(
        ...
        Log.d(
            TAG,
            "feed: inside text feed content $content, contentType: $contentType, tags $tags"
        )
        CoroutineScope(Dispatchers.IO).launch {
            try {

                val feedRequestBody = FeedTextUploadRequestBody(content, contentType, tags)
                val response = retrofitInstance.apiService.uploadTextFeed(
                    feedRequestBody
                )
    ...
    fun uploadFileFeed(filePath: String, content: String, contentType: String, tags: MutableList<String>) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val contentTypePart: RequestBody = content
                    .toRequestBody("text/plain".toMediaTypeOrNull())
                val tagsPart: RequestBody = tags.joinToString(",")
                    .toRequestBody("text/plain".toMediaTypeOrNull())

                val feedRequestBody = FeedTextUploadRequestBody(content, contentType, tags)
//                val tagsRequestBody = tags.map {
//                    val requestBody = it.toRequestBody("text/plain".toMediaTypeOrNull())
//                    MultipartBody.Part.createFormData("tags", it)
//                }.toTypedArray()
                // Convert tags to RequestBody array
                val tagsRequestBody = tags.map {
                    it.toRequestBody("text/plain".toMediaTypeOrNull())
                }.toTypedArray()
//                val response = retrofitInstance.apiService.uploadFilesFeed(
//                    files = image, contentType =contentTypePart, content = contentPart, tags = tagsRequestBody
//                )
//                val responseBody = response.body()
//                Log.d(TAG, "feed: response $response")
//                Log.d(TAG, "feed: response message ${response.message()}")
...
//        val requestFile: RequestBody = RequestBody.create("image/*".toMediaTypeOrNull(), fileBytes)
        ...
        val requestFileThumbnail: RequestBody =
            ProgressRequestBody(thumbnail, "files/*".toMediaTypeOrNull()!!, progressCallback)

        val thumbnailFilePart: MultipartBody.Part =
            MultipartBody.Part.createFormData("feed_thumbnail", thumbnail.name, requestFileThumbnail)
    ...
    suspend fun favoriteFeed(shortOwnerId: String) {
        ...
        try {
            ...
            if (response.isSuccessful) {
                ...
                Log.d(
                    TAG,
                    "isBookmarked ${responseBody?.data!!.isBookmarked}"
                )
            } else {
                Log.d(TAG, "Error: ${response.message()}")
            }
