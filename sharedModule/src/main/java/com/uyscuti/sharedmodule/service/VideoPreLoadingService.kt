package com.uyscuti.sharedmodule.service
...
//    private val simpleCache: SimpleCache = FlashApplication.simpleCache
    private val cache: SimpleCache = FlashApplication.cache

    @OptIn(UnstableApi::class) override fun onHandleIntent(intent: Intent?) {
    ...
    @OptIn(UnstableApi::class) private fun preCacheVideo(videosList: ArrayList<String>?) {
        var videoUrl: String? = null
        if (!videosList.isNullOrEmpty()) {
            videoUrl = videosList[0]
            Log.d("CacheService", "VideoList total: ${videosList.size}")
    ...
    @OptIn(UnstableApi::class) private fun cacheVideo(
        dataSpec: DataSpec,
