...
private const val OPEN_PRODUCT_DETAILS = "com.uyscuti.social.circuit.OPEN_Catalogue_Details"
...
class FeedAdapter(
    ...
    inner class BusinessRepostedPost(val binding: FeedBusinessPostBinding): RecyclerView.ViewHolder(binding.root) {
        ...
        @SuppressLint("SetTextI18n")
        private fun setPostOwnerDetails(data: com.uyscuti.social.network.api.response.posts.Post) {
            Glide.with(context)
            ...
            binding.userName.text = "@${data.author.account.username}"
            binding.fullName.text = "${data.author.firstName} ${data.author.lastName}"
            binding.tvPostTime.text = formattedMongoDateTime(data.createdAt)

            if(currentUsername == data.author.account.username)
                binding.btnFollow.visibility = View.GONE
            else
                setupFollowButton(data)

            CoroutineScope(Dispatchers.IO).launch {
                val response = retrofitInterface.apiService.getSingleAggregatedProduct(data.businessDetails?._id.toString())
                if (response.isSuccessful){
                    businessPost = response.body()?.data[0]
                }
            }
        ...
        private fun setupProductOwner(data: com.uyscuti.social.network.api.response.posts.Post) {
            Glide.with(context)
                .load(data.businessDetails?.author?.account?.avatar?.url)
                .circleCrop()
                .placeholder(R.drawable.ic_person)
                .error(R.drawable.ic_person)
                .into(binding.productOwnerAvatar)

            binding.productOwnerUsername.text = "@${data.businessDetails?.author?.account?.username}"
            binding.productOwnerFullName.text = "${data.businessDetails?.author?.firstName} ${data.businessDetails?.author?.lastName}"
            binding.productPostTime.text = formattedMongoDateTime(data.businessDetails?.createdAt)
            binding.messageUser.text = "Message @${data.author.account.username}"
        }
        ...
        @SuppressLint("SetTextI18n")
        private fun setupProductMedia(data: com.uyscuti.social.network.api.response.posts.Post) {
        ...
        @OptIn(UnstableApi::class)
        private fun setupEventListener(data: com.uyscuti.social.network.api.response.posts.Post) {

            binding.ivUserAvatar.setOnClickListener {
                if (data.author.owner == userId){
                    EventBus.getDefault().post(GoToUserProfileFragment())
                } else {
                    val name = "${data.author.firstName} ${data.author.lastName}"
                    val otherUsersProfile = OtherUsersProfile(
                        name, data.author.account.username,
            ...
            binding.productDetails.setOnClickListener {
                val intent = Intent(OPEN_PRODUCT_DETAILS).apply {
                    putExtra("catalogue", businessPost)
                    putExtra("position", 0)
            ...
            binding.btnFollow.setOnClickListener {
                CoroutineScope(Dispatchers.IO).launch {
                    val response = retrofitInterface.apiService.followUnFollow(data.author.owner)
                    if(response.isSuccessful) {
                        withContext(Dispatchers.Main) {
                            binding.btnFollow.visibility = View.GONE
                            data.isFollowing = true
                        }
                        Log.d("Following", "Started following user")
                    } else {
                        Log.d("Following", "Failed to follow user")
                    }
                }
            }
        ...
        private fun setupDialogManager(data: com.uyscuti.social.network.api.response.posts.Post) {
