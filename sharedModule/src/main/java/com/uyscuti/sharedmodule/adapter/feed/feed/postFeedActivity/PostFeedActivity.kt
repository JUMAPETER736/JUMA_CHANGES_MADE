...
//        if (data.originalPostId == null)
    ...
    private fun generateSampleData(count: Int): List<com.uyscuti.sharedmodule.data.model.shortsmodels.Comment> {
        val itemList = mutableListOf<com.uyscuti.sharedmodule.data.model.shortsmodels.Comment>()
        for (i in 1..count) {
            itemList.add(com.uyscuti.sharedmodule.data.model.shortsmodels.Comment("Item $i"))
        }
        return itemList
    }
