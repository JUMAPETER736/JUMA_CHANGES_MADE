package com.uyscuti.sharedmodule.utils.waveformseekbar
...
import com.uyscuti.sharedmodule.utils.waveformseekbar.utils.ThreadBlocking
import com.uyscuti.sharedmodule.utils.waveformseekbar.utils.Utils
import com.uyscuti.sharedmodule.utils.waveformseekbar.utils.WaveGravity
import com.uyscuti.sharedmodule.utils.waveformseekbar.utils.WaveformOptions
import java.io.File
...
open class WaveformSeekBar @JvmOverloads constructor(
    ...
    var waveGap: Float = Utils.dp(context, 12)
        set(value) {
            field = value
            invalidate()
        }
    ...
    var waveWidth: Float = Utils.dp(context, 6)
        set(value) {
            field = value
            invalidate()
        }
    ...
    var waveCornerRadius: Float = Utils.dp(context, 6)
        set(value) {
            field = value
            invalidate()
        }
        ...
        if (visibleProgress > 0) {
            when (event?.action) {
                ...
                MotionEvent.ACTION_UP -> {
                    updateOnRelease(event) // Call your custom method here
                    performClick()
                }
            }
        } else {
    ...
    private fun updateOnRelease(event: MotionEvent?) {
        ...
        // For example, you might want to notify a listener or update the UI
        progress = event?.let { getProgress(it) }!!
        onProgressChanged?.onRelease(event, progress)
    }
