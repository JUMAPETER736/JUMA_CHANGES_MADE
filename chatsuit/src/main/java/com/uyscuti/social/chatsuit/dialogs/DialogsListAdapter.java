...
package com.uyscuti.social.chatsuit.dialogs;
...
public class DialogsListAdapter<DIALOG extends IDialog>
    ...
    public void updateItemById(DIALOG item) {
        ...
        if (position != -1) {
            ...
            // If the item is not found, you may want to handle this case accordingly.
            // For example, add the item to the list if it doesn't exist.
            items.add(item);
            notifyItemInserted(items.size() - 1);
        }
    }
    ...
    public void updateItemBy(DIALOG item) {
        if (items == null) {
            items = new ArrayList<>();
        }
        ...
        if (position != -1) {
            ...
            if (items.size() == 1) {
                // If there's only one item, notify item inserted at position 0
                notifyItemInserted(0);
            } else {
                // If there are more than one items, notify item inserted at the end of the list
                notifyItemInserted(items.size() - 1);
            }
        }
    }
    ...
    public void updateItemId(DIALOG item) {
        if (items == null) {
            items = new ArrayList<>();
        }
        ...
        if (position != -1) {
            ...
            if (items.size() == 1) {
                // If there's only one item, use notifyItemInserted
                notifyItemInserted(0);
            } else {
                // If there are more than one items, use notifyItemInserted with the proper position
                notifyItemInserted(items.size() - 1);
            }
        }
    }
    ...
    public void resetSelectionForAll() {
        if (items != null) {
            for (int i = 0; i < items.size(); i++) {
                DIALOG item = items.get(i);
    ...
    public void upsertItemW(DIALOG item) {
        boolean updated = false;
        List<DIALOG> updatedList = new ArrayList<>(items); // Create a new list

        for (int i = 0; i < updatedList.size(); i++) {
            if (updatedList.get(i).getId().equals(item.getId())) {
        ...
        items = updatedList; // Set the updated list as the new list
    }
    ...
    public static class DialogViewHolder<DIALOG extends IDialog> extends BaseDialogViewHolder<DIALOG> {
        ...
        private void applyStyle() {
            if (dialogStyle != null) {
                ...
                if (tvBubble != null) {
                    ...
                    tvBubble.setTypeface(tvBubble.getTypeface(), Typeface.NORMAL);
                }
            }
        }
        ...
        private void applyDefaultStyle() {
            if (dialogStyle != null) {
                ...
                if (tvDate != null) {
                    ...
                    tvDate.setTypeface(Typeface.DEFAULT, Typeface.NORMAL);
                }

                if (tvLastMessage != null) {
                    tvLastMessage.setTextColor(dialogStyle.getDialogMessageTextColor());
                    tvLastMessage.setTypeface(Typeface.DEFAULT, Typeface.NORMAL);
                }
            }
        }
        ...
        public void onBind(final DIALOG dialog) {
            if (dialog.getUnreadCount() > 0 || dialog.getIsSelected()) {
                applyUnreadStyle();
            } else  {
                applyDefaultStyle();
            }
...
//                Log.d("Formatter", "Last Message Date In Dialog List adapter "  +  lastMessageDate);

//                datesFormatter.format(lastMessageDate);
...
//                Log.d("Formatter", "No Last Message Date Found");
                tvDate.setText(null);
            }
