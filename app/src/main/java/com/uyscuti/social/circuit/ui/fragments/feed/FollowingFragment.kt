...
import com.uyscuti.social.circuit.ui.feedactivities.ReportNotificationActivity2
...
import com.uyscuti.social.circuit.ui.fragments.feed.feedviewfragments.FeedDocumentViewFragment
...
import com.uyscuti.social.circuit.ui.fragments.CommentsBottomSheet
    ...
    fun getAllFeed(page: Int) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                Log.d(TAG, "Feed Feed getAllFeed feed: response message " +
                        "${response.message()}")
                Log.d(TAG, "Feed Feed getAllFeed feed: response message error body" +
                        " ${response.errorBody()}")
                Log.d(TAG, "Feed Feed getAllFeed feed: response body $responseBody")

                Log.d("AllFeedTag", "Feed Feed getAllFeed feed: response body message" +
                ...
                if (posts.size > 4) {
                    Log.d(TAG, "Feed Feed getAllFeed feed: response body data" +
                            " ${posts[4]}")
                } else {
                    Log.d(TAG, "Feed Feed getAllFeed feed: Not enough posts," +
                            " size=${posts.size}")
                }
                Log.d(TAG, "Feed Feed getAllFeed feed: response body data " +
                        "${data.posts.posts.size}")
                withContext(Dispatchers.Main) {
                    ...
                    followedPostsAdapter.submitItems(posts)
                }
...
private fun Fragment_Original_Post_With_Repost_Inside?.setListener(
    fragment: com.uyscuti.social.circuit.ui.fragments.feed.FollowingFragment
) {
}
