package com.uyscuti.social.circuit.bottomSheet;
...
public class  BottomNavigationView extends RoundedCornerLayout implements NavigationItem.OnSelectorColorChangedListener {
            ...
            if (lastPosition != position) {
                if (animationType == ANIM_TRANSLATE_X) {
                    if (initialPosition != -1)
                        v_selectors.get(initialPosition).setVisibility(View.INVISIBLE);
                    v_selectors.get(position).setVisibility(View.VISIBLE);
    ...
    // Add this method to force a view recreation
    public void forceRecreate() {
        Log.d("Force Recreate", "forceRecreate: ");
        invalidate();
        requestLayout();
    }
    ...
    private final List<View> v_selectors = new ArrayList<>();
    private final List<NavigationItem> buttons = new ArrayList<>();
    ...
    private void initViews() {
       ...
       BackgroundView backgroundView = new BackgroundView(getContext());
        ...
        v_bottomLine.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, 3));
        addView(v_bottomLine);
    }

    private void setBottomLineAttrs() {
        LayoutParams layoutParams = (LayoutParams) v_bottomLine.getLayoutParams();
    ...
    private void setSelectorAttrs() {
        LayoutParams selectorParams = (LayoutParams) selectorContainer.getLayoutParams();
        LayoutParams bottomLineParams = (LayoutParams) v_bottomLine.getLayoutParams();
    ...
    private void getAttributes(AttributeSet attrs) {
        ...
        checkedButtonId = ta.getResourceId(R.styleable.BottomNavigationView_nav_checkedButton, View.NO_ID);
    ...
    public void removeBadge(int position) {
        ...
        // Check if the badge is present and remove it
        assert item != null;
        item.removeViewAt(2);

    }
    private NavigationItem findItemByPosition(int position) {
        Log.d("Badge", "(findItemByPosition) position - " + position);

        if (position >= 0 && position < buttons.size()) {
            return buttons.get(position);
        } else {
            Log.d("Badge", "(findItemByPosition) position not found");
    ...
    public void addItem(NavigationItem item) {
        ...
        Log.d(TAG, "id: " +id);
        if (lastPosition == -1) {
            if (checkedButtonId != View.NO_ID && checkedButtonId == id)
                lastPosition = initialPosition = position;
            else if (checkedButtonId == View.NO_ID && item.isChecked())
                lastPosition = initialPosition = position;
        }
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        if (child instanceof NavigationItem) {
            ...
            if (lastPosition == -1) {
                if (checkedButtonId != View.NO_ID && checkedButtonId == id)
                    lastPosition = initialPosition = position;
                else if (checkedButtonId == View.NO_ID && button.isChecked())
                    lastPosition = initialPosition = position;
            }
    ...
    private void animateSelectorSliding(int toPosition, String property, boolean hasAnimation, boolean enableDeselection) {
        ...
        if (!isViewDrawn) {
            if (initialPosition != -1)
                v_selectors.get(initialPosition).setVisibility(View.INVISIBLE);
            v_selectors.get(toPosition).setVisibility(View.VISIBLE);
    ...
    public void setOnLongClickedButtonListener(final OnLongClickedButtonListener onLongClickedButtonListener) {
        for (int i = 0; i < numberOfButtons; i++) {
            ...
            buttons.get(i).setOnLongClickListener(new OnLongClickListener() {
                @Override
